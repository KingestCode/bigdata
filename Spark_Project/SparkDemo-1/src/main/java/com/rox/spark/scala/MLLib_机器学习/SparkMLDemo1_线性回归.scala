...
/*    // 数据目录
    ...
    lr.setMaxIter(50)
    //通过线性回归拟合训练数据，生成模型
    val mymodel = lr.fit(trainingDF)

    // 保存模型
    val modelpath = "file:///Users/shixuanji/Documents/IDEs/iTerm2/scala/model"
    ...
    // 创建内存 测试数据 框 (可以从训练数据集中截取数据, 这样会算的比较接近)
    ...
    //
    println("======================")
    val featureDF: DataFrame = sess.sql("SELECT features FROM test")

    //对特征数据进行模型变换，得到预测结果
    val predictedDF: DataFrame = model.transform(featureDF).select("features", "prediction")
