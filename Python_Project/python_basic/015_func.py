...
# 如果一个函数没有定义 return 或者 只有 return 
# 没有返回值的话, 那函数的返回值就是 None
...
按传值的方式复制了变量 b ，传递的只是 b 的值，并没有影响到 b 的本身。
"""
## 有意思的测试：
def chagne_number( b ):
    b = 1000
...
## 关键字参数
print_user_info(name="huangbo", age=30,  sex="M")
"""
需要注意的是：
1、只有在形参表末尾的那些参数可以有默认参数值
2、默认参数的值是不可变的对象，比如None、True、False、数字或字符串，
当默认参数是可变对象时，默认值在其他地方被修改后你将会遇到各种麻烦。这些修改会影响到下次调用这个函数时的默认值。
"""
