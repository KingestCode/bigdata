...
object PageOneStepConvertRate {
  ...
  /**
    * 计算页面切片转化率
    * @param pageSplitPvMap 页面切片pv - 每个跳转切片的总个数
    * @param startPagePv 起始页面pv
    * @return convertRateMap 存储 目标所要求的 所有转化率的字典
    */
  ...
  :collection.Map[String, Double] = {
    ...
    for(targetPage <- targetPagePairs){
      // 先获取pageSplitPvMap(每个跳转切片的总个数) 中记录的当前targetPage的数量, .get 是因为是一个 option
  ...
  :Long = {
    ...
    val startPageRDD: RDD[Long] = sessionid2actionsRDD.flatMap{
    ...
    // 对PageId等于startPageId的用户行为数据进行计算
    startPageRDD.count()
  }
  ...
  :RDD[(String, Int)] = {
    ...
    val targetPagePairs: List[String] = targetPages.slice(0, targetPages.length-1).zip(targetPages.tail).map(item => item._1 + "_" + item._2)
    ...
    sessionid2actionsRDD.flatMap{
      ...
      // 按照时间排序
      val sortedUVAs: List[UserVisitAction] = userVisitActions.toList.sortWith((uva1, uva2) => DateUtils.parseTime(uva1.action_time).getTime() < DateUtils.parseTime(uva2.action_time).getTime())
      ...
      // 输出：(1_2, 1) (5_6, 1) (6_7, 1)
        // filter 中 过滤的是 list 中的每个元素
        // 过滤出: 目标 list 中的值, 包含当前session的PageFlow 中的值, 的情况.; 再拼1
