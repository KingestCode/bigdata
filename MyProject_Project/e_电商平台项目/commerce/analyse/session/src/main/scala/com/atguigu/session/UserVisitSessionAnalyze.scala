...
object UserVisitSessionAnalyze {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // filteredSessionid2AggrInfoRDD 是按照年龄、职业、城市范围、性别、搜索词、点击品类这些条件过滤 sessionid2AggrInfoRDD 后的最终结果. sessionAggrStatAccumulator累加器会在此次过滤的过程中赋值, 依然返回 <sessionid,fullAggrInfo>
  ...
  def getTop10Session(spark: SparkSession, taskid: String, top10CategoryList: Array[(CategorySortKey, String)], sessionid2ActionRDD: RDD[(String, UserVisitAction)]) {
    ...
    // 获得所有需要求的category集合
    val top10CategoryIdRDD = spark.sparkContext.makeRDD(top10CategoryList.map {
    ...
    val categoryid2sessionCountRDD: RDD[(Long, String)] = sessionid2ActionsRDD.flatMap {
      ...
      // 对categoryCountMap中的数据进行格式转化 (categoryid, "sessionid,count") sessionid+点击次数
    ...
    val top10CategorySessionCountRDD = top10CategoryIdRDD.join(categoryid2sessionCountRDD).map {
      case (cid, (ccid, value)) => (cid, value)
    }

    // 第三步：分组取TopN算法实现，获取每个品类的top10活跃用户

    // 先按照品类分组, 每一个 category 和 点击过它的 session 和对应的点击次数
    ...
    val top10SessionObjectRDD: RDD[Top10Session] = top10CategorySessionCountsRDD.flatMap {
      case (categoryid, clicks) =>
      // 先排序，然后取前10          List((sessionid,count),(sessionid,count),...),  list 内部按照点击次数count , 从大到小排序
    ...
    val sessionDetailRDD = top10SessionRDD.join(sessionid2ActionRDD).map {
  ...
  : RDD[(Long, Long)] = {
    ...
    // 只将点击行为过滤出来
    val clickActionRDD = sessionid2detailRDD.filter {
      case (sessionid, userVisitAction) => userVisitAction.click_category_id != null
    }

    // 获取每种类别的点击次数
    // map阶段：(品类ID，1L) 一个 action 可以转为一个 (Long,Long)
  ...
  : RDD[(Long, Long)] = {
    ...
    // 获取每种类别的下单次数
    // flatMap  一个 action 可以转为多个 (Long,Long)
  ...
  : Array[(CategorySortKey, String)] = {
    ...
    // 第三步：join各品类与它的点击、下单和支付的次数 (value: RDD[(Long, String) 订单品类 id, 各种次数和的string)
    // distinctCategoryIdRDD 中是所有产生过点击、下单、支付行为的商品类别
    // 通过distinctCategoryIdRDD与各个统计数据的LeftJoin保证数据的完整性
    //  数据格式: (78,categoryid=78|clickCount=99|orderCount=23|payCount=32)
  ...
  def randomExtractSession(spark: SparkSession, taskUUID: String, sessionid2AggrInfoRDD: RDD[(String, String)], sessionid2actionRDD: RDD[(String, UserVisitAction)]) {
    ...
    def hourExtractMapFunc(hourExtractMap: mutable.HashMap[String, mutable.ListBuffer[Int]],
      ...
      for ((hour, count) <- hourCountMap) {
        ...
        // sxj 避免一个小时要抽取的数量 > 此小时的总数量
        if (hourExtractNumber > count) {
          hourExtractNumber = count.toInt
        }

        // 仍然通过模式匹配实现有则追加，无则新建, 填充要取的随机值 map, 格式(hour, List(1,2,4,5,3,6...))
    ...
    val sessionRandomExtract: RDD[SessionRandomExtract] = time2sessionsRDD.flatMap { case (dateHour, items) =>
      ...
      // 从广播变量中提取出数据 dateHourExtractMap[天，[小时，List(random...,...)]]
      ...
      for (sessionAggrInfo <- items) {
        // 如果筛选List(生成的List(随机数字)) 中包含当前的index，则提取此sessionAggrInfo中的数据
  ...
  /**
    * 获取通过筛选条件的session的访问明细数据RDD (需求1)
    *
    * @param sessionid2aggrInfoRDD
  ...
  def aggregateBySession(spark: SparkSession, sessinoid2actionRDD: RDD[(String, UserVisitAction)]): RDD[(String, String)] = {
    ...
    val sessionid2FullAggrInfoRDD = userid2FullInfoRDD.map {
      ...
      // 由此格式 => (sessionid, fullAggrInfo)此格式
  ...
  def getActionRDDByDateRange(spark: SparkSession, taskParam: JSONObject): RDD[UserVisitAction] = {
    ...
    import spark.implicits._
    // 拼接变量时, 单引号中加包双引号, 双引号中 +变量+, sql: "  '"+变量+"'  "
