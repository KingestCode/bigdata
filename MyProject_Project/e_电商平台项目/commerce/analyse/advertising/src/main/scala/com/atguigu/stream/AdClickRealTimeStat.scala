...
object AdClickRealTimeStat {
  ...
  def calculateProvinceTop3Ad(spark:SparkSession, adRealTimeStatDStream:DStream[(String, Long)]) {
    ...
    val rowsDStream = adRealTimeStatDStream.transform { rdd =>
      ...
      val rowsRDD: RDD[(String, String, Long, Long)] = dailyAdClickCountByProvinceRDD.map{
  ...
  :DStream[(String, Long)] = {
    ...
    // spark streaming 特有的一种算子，在spark集群内存中，维护一份key的全局状态
    ...
    // 每次都从 checkpoint 中取出, 加上当前的值, 在放回checkpoint; 下次再取出.  当前的       存在 checkpoint 中的
  ...
  def filterByBlacklist(spark: SparkSession, adRealTimeValueDStream:DStream[String]):DStream[(Long, String)] = {
    ...
    val filteredAdRealTimeLogDStream = adRealTimeValueDStream.transform{
    ...
    /*
    过滤黑名单数据另一种方法: 直接判断 数据中的 userId 字段是否包含在 黑名单数组 中, 取不包含的
